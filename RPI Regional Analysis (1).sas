...
ods graphics on;
proc reg data=wage plots=diagnostics;
model exp_change = med_earn jobs males females gvsu_comp diversity/ CLB CLM CLI;
ID desc;
title "Effects on Expected Change in occupation";
title2 "Nicholas Stambaugh";

ods graphics on;
proc reg data=wage plots=diagnostics;
model jobs = gvsu_comp diversity males females exp_change med_earn/ CLB CLM CLI;
ID desc;
title "Effects on Jobs by occupation";
title2 "Nicholas Stambaugh";
